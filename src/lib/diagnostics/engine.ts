// Motor de diagnóstico baseado em regras
...
export function runDiagnostics({ store, products, reviews, competitors, campaigns }: DiagnosticInput): Diagnostic[] {
  ...
  if (store.average_ticket && store.average_ticket < 35) {
    diags.push({
      ...
      business_impact: "Perde receita por pedido — opera com mais pedidos para o mesmo faturamento",
  ...
  if (fasterCompetitors.length >= 2) {
    diags.push({
      ...
      probable_cause: "Operação ou raio menos eficiente que a concorrência",
      business_impact: "Cliente escolhe quem entrega mais rápido em horários de fome alta",
      recommended_solution: "Reduzir promessa de tempo e cumprir; estudar operação dos concorrentes",
      priority: "media",
  ...
  if (topIssues.length) {
    diags.push({
      ...
      probable_cause: "Falha operacional sistemática (não pontual)",
...
export const scoreLabel = (score: number) =>
  score >= 80 ? "Bom" : score >= 60 ? "Atenção" : "Crítico";
