    ...
    <div className="space-y-6">
      ...
      <Card className="p-8 text-center shadow-elegant">
        ...
        <p className="text-sm text-muted-foreground mt-3 max-w-xl mx-auto">
          {overall >= 80 ? "Sua loja está em ótima condição. Foque em otimizações finas e crescimento."
            : overall >= 60 ? "Sua loja tem bom potencial mas há áreas críticas para atacar."
        ...
        {Object.entries(areas).map(([area, score]) => {
          ...
          return (
            <Card key={area} className="p-4 shadow-card">
              ...
              <p className="text-xs text-muted-foreground mt-1">{scoreLabel(score)}</p>
            </Card>
          );
        })}
      </div>
